...
SNIPER_SIZE = 8
SNIPER_PIERCE = True
# пуля снайпера набирает скорость в полёте (не постоянная, как у остальных
# видов оружия) — читается как "разгоняющийся бронебойный снаряд", отличает
# её и по ощущению, не только по цифре урона/пробитию. Множитель применяется
# КАЖДЫЙ тик, пока не достигнут потолок SNIPER_MAX_SPEED_MULT (иначе пуля
# ускорялась бы неограниченно на длинной дистанции полёта).
SNIPER_ACCEL_PER_TICK = 1.03
SNIPER_MAX_SPEED_MULT = 1.8

# Ближний бой: полноценный дробовик — залп мелких снарядов веером за один
# выстрел, урон падает с дистанцией. Раньше было 2 параллельных ствола без
# разброса — не читалось как дробовик, только как двустволка.
BRAWLER_COOLDOWN = 0.55
BRAWLER_PELLET_COUNT = 6  # снарядов в одном залпе
BRAWLER_PELLET_DAMAGE = 10  # урон ОДНОЙ дробины в упор — все 6 в упор ~ старый суммарный урон двустволки (52)
BRAWLER_MIN_DAMAGE_MULT = 0.35
BRAWLER_MAX_RANGE = 420.0
BRAWLER_SPREAD = 0.5  # радианы, полная ширина веера — заметно шире прежних 0.12 (два параллельных ствола)
BRAWLER_SPEED = 760.0
BRAWLER_SIZE = 8  # мельче прежних 11 — дробины, не единичные снаряды
...
TRAP_COUNT = 4  # сколько ловушек одновременно на карте
TRAP_ROTATION_INTERVAL = 10.0  # сек между заменой одной ловушки на новую в случайном месте
...
# длина луча больше не константа — вычисляется динамически до границы арены
# (ray_distance_to_field_edge в map.py), см. _fire_miniboss_laser в miniboss.py
...
# вращающихся вокруг танка на 360° за LASER_STAR_DURATION (ровно один полный
# оборот, не более — потом гаснут), тикают урон по всем, кого касаются, всё
# время действия (не требует зажатия кнопки). Длина луча не фиксирована
# константой — считается в room.py как половина диагонали поля (см.
# LASER_STAR_RANGE ниже, вычисляется через FIELD_WIDTH/FIELD_HEIGHT из map.py),
# чтобы луч доставал до самого дальнего угла арены из любой точки, где стоял
# игрок в момент подбора, но никогда не вылезал за пределы карты.
LASER_STAR_DURATION = 3.5
...
LASER_STAR_TICK_DAMAGE = 9  # тот же тик-урон, что и раньше — короче стало окно действия, не сила тика
...
class Player:
    ...
    laser_star_started_at: float = 0.0  # момент подбора — точка отсчёта для вращения (ровно 360° за LASER_STAR_DURATION)
    laser_star_base_angle: float = 0.0  # угол первого луча в момент подбора, остальные 7 смещены равномерно от него
    laser_star_angles: list = field(default_factory=list)  # текущие 8 углов — пересчитываются каждый тик в _process_laser_star (вращение), не статичны
...
class Bullet:
    ...
    speed_mult: float = 1.0  # накопленный множитель скорости — растёт у sniper, см. SNIPER_ACCEL_PER_TICK
