...
# Точка спавна мини-босса — раньше он появлялся ровно там, где умер игрок
# (см. вызов _maybe_spawn_miniboss в room.py), а это могло оказаться вплотную
# к стене/укрытию. Мини-босс (MINIBOSS_SIZE=96) при этом не умеет объезжать
# препятствия (см. _drive_miniboss_ai в miniboss.py — идёт по прямой к
# waypoint без pathfinding), и если стена оказывалась прямо между ним и
# случайным waypoint, он утыкался в неё и стоял намертво (waypoint
# перевыбирается только когда босс УЖЕ дошёл, то есть никогда). Точка ниже
# подобрана программным перебором как максимально удалённая от любой стены
# И любой ямы во всей сетке карты (176px запаса до ближайшей стены, 221px до
# ямы — при полуразмере босса 48px это большой гарантированный зазор), чтобы
# первый шаг ИИ после спавна проходил по открытому пространству.
MINIBOSS_SPAWN_POINT: tuple[float, float] = (430.0, 410.0)
